 ...
 const MeetingList = ({meetings,type}) => {
   ...
   return <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-3'>
       {meetings.map((meeting)=>{
        return(
<Card key={meetings.id} className="flex flex-col justify-between">
  <CardHeader>
    <CardTitle>{meeting.event.title}</CardTitle>
    <CardDescription>with {meeting.event.name}</CardDescription>
    <CardDescription>&quot;{meeting.additionalInfo}&quot</CardDescription>
  </CardHeader>
  <CardContent>
